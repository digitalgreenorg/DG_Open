...
class CLI:
    ...
    @classmethod
    def colorize(cls, message, color=COLOR_DEFAULT):
        return '{}{}{}'.format(color, message, cls.NO_COLOR)

    @classmethod
    def framed_print(cls, message, color=NO_COLOR, columns=70):
        border = '═' * (columns - 2)
        framed_message = [
            '╔{}╗'.format(border),
            '║ {} ║'.format(' ' * (columns - 4)),
    ...
    def get_response(cls, validators=None, default='', to_lower=True,
        ...
        while True:
            try:
                response = cls.colored_input('Enter your choice : ', cls.COLOR_INFO, default)
                ...
                cls.colored_print("Value Exception. Try Again!",
                                cls.COLOR_ERROR)
    ...
    def run_command(cls, command, cwd=None, polling=False):
        if polling:
            ...
            return stdout
# cli = CLI()
# response = CLI.run_command(command='brew uinstall loki')
# print(response)